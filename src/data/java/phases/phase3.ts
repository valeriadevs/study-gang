...
export const phase3days: Day[] = [
  // DAY 7: Classes, Objects, Constructors
  {
    ...
    blocks: [
      { type: 'callout', id: 'd7-intro', calloutType: 'info', title: 'Welcome to OOP', content: 'OOP is a **way of thinking** about code as interacting objects. Class = blueprint. Object = instance. Constructor = initializer. Today: the foundation of everything from Day 8-14.' },
      ...
      { type: 'list', id: 'd7-con-rules', listStyle: 'number', items: ['Same name as class, **NO return type** (not even void!).', 'If you write NO constructor → Java provides default no-arg constructor.', 'If you write ANY constructor → default constructor **DISAPPEARS**.', 'Can be overloaded. Use `this()` for constructor chaining (must be first line).', 'If you add void → it becomes a regular method, NOT a constructor!'] },
      ...
      { type: 'list', id: 'd7-this-list', items: ['**this.fieldName**: Disambiguates instance vars from params with same name.', '**this.methodName()**: Calls another method on same object (rare, implicit).', '**this()**: Calls another constructor (constructor chaining). Must be first line.'] },
      // Doubt
      { type: 'callout', id: 'd7-d1', calloutType: 'doubt', title: "What if I don't write ANY constructor?", content: 'Java provides a **default no-arg constructor** automatically: `public Person() {}`. But it **disappears** the moment you write any constructor. If you need both a parameterized AND a no-arg, you must write both explicitly.' },
      { type: 'callout', id: 'd7-d2', calloutType: 'doubt', title: 'Why use getters/setters instead of public fields?', content: '**Encapsulation**: 1) Validate data (reject negative age). 2) Change internal representation without breaking external code. 3) Make fields read-only (getter only, no setter). This pattern is tested in both theory and coding questions.' },
      // Exam
      { type: 'callout', id: 'd7-exam', calloutType: 'exam', title: 'Exam Alert', content: '1. **Constructor has NO return type** — not even void. Adding void makes it a regular method.\n2. **Default constructor disappears** when you write your own.\n3. **this() must be first line** in constructor chaining.\n4. **this.field vs parameter** — disambiguation is tested in code-tracing.' },
      // Bridge
      { type: 'callout', id: 'd7-bridge', calloutType: 'bridge', title: 'Connect the Dots', content: 'Constructors + `this` → **super() (Day 8)** for parent constructors. Getters/setters → **encapsulation** appears in every OOP design question. Constructor overloading → **method overloading (Day 9)**.' },
      // Quick Ref
  ...
  // DAY 8: Static & Inheritance
  {
    ...
    blocks: [
      { type: 'callout', id: 'd8-intro', calloutType: 'info', title: 'Shared State and Code Reuse', content: '`static` members belong to the **class itself**, not individual objects. **Inheritance** lets a child class acquire everything from its parent. Together, they form the backbone of code organization.' },
      { type: 'heading', id: 'd8-static', level: 2, content: 'Static — Belonging to the Class' },
      { type: 'code', id: 'd8-static-code', lang: 'java', title: 'Static Counter Example', code: 'class Student {\n    String name;\n    static int count = 0;\n    public Student(String n) { name=n; count++; }\n    static int getCount() { return count; }\n    // static method CANNOT use this or access instance vars!\n}' },
      { type: 'callout', id: 'd8-static-rules', calloutType: 'warn', title: 'Static Method Rules', content: '1. Can ONLY directly access static members.\n2. CANNOT use `this` (no object context).\n3. Instance methods CAN access both static and instance members.\n4. `main` is static because JVM calls it without creating an object.' },
      ...
      { type: 'callout', id: 'd8-super', calloutType: 'tip', title: 'super — Three Uses', content: '1. **super()**: Call parent constructor (must be first line).\n2. **super.method()**: Call parent\'s version of overridden method.\n3. **super.field**: Access parent\'s field (when shadowed).' },
      { type: 'callout', id: 'd8-diamond', calloutType: 'warn', title: 'No Multiple Class Inheritance', content: 'Java does NOT allow `class C extends A, B`. This avoids the **Diamond Problem**: if A and B both have `doSomething()`, which one does C inherit? Solved by **interfaces (Day 10)**.' },
      // Doubt
      { type: 'callout', id: 'd8-d1', calloutType: 'doubt', title: "Why can't static methods use this?", content: '`this` refers to the **current object instance**. Static methods belong to the **class** — there is no object, so `this` has no meaning. Think of it this way: you can call `Student.getCount()` without ever creating a Student object. What would `this` refer to? Nothing.' },
      { type: 'callout', id: 'd8-d2', calloutType: 'doubt', title: 'Can a subclass access private members of its parent?', content: '**No.** `private` means private. Subclasses cannot directly access private fields/methods of the parent. Use `protected` if you want subclass access. The subclass inherits them (they exist in memory) but cannot access them directly — must use public/protected getters.' },
      // Exam
      { type: 'callout', id: 'd8-exam', calloutType: 'exam', title: 'Exam Alert', content: '1. **Java does NOT support multiple class inheritance** — #1 OOP MCQ.\n2. **super() must be first line** in constructor (same as this()).\n3. **Static methods cannot be overridden**, only hidden.\n4. **protected** allows subclass access even from different packages.' },
      // Bridge
      { type: 'callout', id: 'd8-bridge', calloutType: 'bridge', title: 'Connect the Dots', content: 'Method overriding (brief today) → **Polymorphism (Day 9)** full coverage. `super` → used extensively when overriding methods. Inheritance hierarchies → **abstract classes (Day 10)** formalize parent classes.' },
      // Quick Ref
      { type: 'table', id: 'd8-qref', headers: ['Concept', 'Key Point'], rows: [['static field', 'One copy shared by all instances. ClassName.field.'], ['static method', 'No this. Only access static members. Called via class.'], ['extends', 'Child inherits all non-private members from parent.'], ['super()', 'Call parent constructor. Must be first line.'], ['Multiple inheritance', 'NOT supported for classes. Supported via interfaces.']] },
      // Quiz
  ...
  // DAY 9: Polymorphism
  {
    ...
    blocks: [
      { type: 'callout', id: 'd9-intro', calloutType: 'info', title: 'Polymorphism — "Many Forms"', content: 'Same name, different behavior. Two types: **compile-time** (overloading) and **runtime** (overriding with dynamic dispatch). Guaranteed CE-2 topic — the comparison table is essential.' },
      { type: 'heading', id: 'd9-overload', level: 2, content: 'Method Overloading (Compile-Time)' },
      { type: 'code', id: 'd9-ol-code', lang: 'java', title: 'Overloading', code: 'class MathUtils {\n    int add(int a, int b) { return a+b; }         // 2 ints\n    int add(int a, int b, int c) { return a+b+c; } // 3 ints (diff count)\n    double add(double a, double b) { return a+b; }  // 2 doubles (diff type)\n    double add(int a, double b) { return a+b; }     // mixed types\n}' },
      { type: 'callout', id: 'd9-ol-rules', calloutType: 'info', title: 'Overloading Rules', content: '1. Parameter list MUST differ (count/type/order).\n2. Return type alone is NOT enough.\n3. Can change access modifier.\n4. Can overload static methods.\n5. Happens in same class (or parent-child via inheritance).' },
      ...
      { type: 'table', id: 'd9-comp-table', headers: ['Feature', 'Overloading', 'Overriding'], rows: [['Class', 'Same class', 'Parent↔Child'], ['Signature', 'MUST differ', 'MUST be identical'], ['Return', 'Can differ', 'Same or covariant'], ['Access', 'Can differ', 'Cannot be more restrictive'], ['Binding', 'Compile-time', 'Runtime'], ['Keyword', 'None needed', '@Override (recommended)'], ['static', 'Can overload', 'Cannot override (hidden)'], ['final', 'Can overload', 'Cannot override'], ['Polymorphism', 'Static/Compile-time', 'Dynamic/Runtime']] },
      { type: 'callout', id: 'd9-binding', calloutType: 'tip', title: 'Static vs Dynamic Binding', content: '**Static binding**: Compiler knows exact method at compile time. Used for: overloaded, static, private, final methods.\n**Dynamic binding**: JVM decides at RUNTIME based on actual object type. Used for: overridden methods. `Animal a = new Dog(); a.sound();` → JVM calls Dog\'s version.' },
      // Doubt
      { type: 'callout', id: 'd9-d1', calloutType: 'doubt', title: 'Can I overload by changing only the return type?', content: '**NO.** The compiler error is: "method is already defined." The JVM cannot tell which version you want just from the return type. Parameter list MUST differ.' },
      { type: 'callout', id: 'd9-d2', calloutType: 'doubt', title: 'Can I prevent a method from being overridden?', content: 'Yes — three ways:\n1. **final** method: `public final void sound() {}`\n2. **final** class: no subclass can exist to override anything.\n3. **private** method: not inherited, so cannot be overridden (can be "redefined" in child, but that is not overriding).' },
      // Exam
      { type: 'callout', id: 'd9-exam', calloutType: 'exam', title: 'Exam Alert', content: '1. **The comparison table** is a guaranteed question. Memorize it.\n2. **Overloading = compile-time. Overriding = runtime.** ALWAYS.\n3. **@Override annotation** catches signature mistakes at compile time.\n4. **Dynamic dispatch**: parent reference + child object = child\'s method runs.' },
      // Bridge
      ...
      // Quick Ref
      { type: 'table', id: 'd9-qref', headers: ['Concept', 'Key Point'], rows: [['Overloading', 'Same name, different params. Compile-time.'], ['Overriding', 'Same signature, child redefines parent. Runtime.'], ['@Override', 'Optional but recommended. Catches errors.'], ['Dynamic dispatch', 'JVM uses actual object type, not reference type.']] },
      // Quiz
  ...
  // DAY 10: Abstraction & Interfaces
  {
    ...
    blocks: [
      { type: 'callout', id: 'd10-intro', calloutType: 'info', title: 'Designing Contracts', content: '**Abstract class**: partial implementation — some methods defined, some left abstract. **Interface**: pure contract — defines WHAT, not HOW. Both enable polymorphism. Today: when to use each.' },
      ...
      { type: 'code', id: 'd10-if-code', lang: 'java', title: 'Multiple Interfaces', code: 'interface Flyable { void fly(); }\ninterface Swimmable { void swim(); }\nclass Duck implements Flyable, Swimmable {\n    public void fly() { System.out.println("Duck flies"); }\n    public void swim() { System.out.println("Duck swims"); }\n}\n// A class can implement MANY interfaces — solves diamond problem!' },
      { type: 'heading', id: 'd10-compare', level: 2, content: 'Abstract Class vs Interface' },
      { type: 'table', id: 'd10-comp-table', headers: ['Feature', 'Abstract Class', 'Interface'], rows: [['Keyword', 'abstract class', 'interface'], ['Methods', 'Abstract + concrete', 'Abstract + default/static (Java 8+)'], ['Variables', 'Instance variables OK', 'Only constants (public static final)'], ['Constructors', 'Can have', 'Cannot have'], ['Multiple', 'Extend ONE only', 'Implement MANY'], ['Use when...', 'Shared state/behavior (is-a)', 'Shared capability (can-do)']] },
      { type: 'callout', id: 'd10-diamond-solved', calloutType: 'tip', title: 'Diamond Problem — Solved', content: 'A class can implement multiple interfaces. If two interfaces have the same default method, the class MUST explicitly override to resolve ambiguity: `InterfaceA.super.method()`. This forces conscious resolution instead of silent ambiguity.' },
      // Doubt
      { type: 'callout', id: 'd10-d1', calloutType: 'doubt', title: 'When should I use abstract class vs interface?', content: '**Decision framework**:\n- Classes share state (fields) and behavior (concrete methods)? → **Abstract class**\n- Unrelated classes need to share a capability? → **Interface**\n- Need constructors or non-public methods? → **Abstract class**\n- Need multiple inheritance of type? → **Interface**\nIn modern Java, interfaces with default methods blur this line. The rule of thumb: IS-A (abstract) vs CAN-DO (interface).' },
      { type: 'callout', id: 'd10-d2', calloutType: 'doubt', title: 'Can an interface have a constructor?', content: '**No.** Interfaces cannot be instantiated, so constructors have no purpose. Fields in interfaces are implicitly `public static final` (constants). If you need instance state and constructors, use an abstract class.' },
      // Exam
      { type: 'callout', id: 'd10-exam', calloutType: 'exam', title: 'Exam Alert', content: '1. **1 abstract class, MANY interfaces** — solves multiple inheritance.\n2. **Serializable, Cloneable, Comparable** — these are interfaces, not classes.\n3. **Abstract class CAN have constructors** (called via super() from subclass).\n4. **Interface CANNOT have constructors** — prevents instantiation.' },
      // Bridge
