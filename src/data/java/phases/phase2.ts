...
static int makeScore() {
  ...
  // ======== DAY 6: Arrays & Strings ========
  {
    ...
    blocks: [
      ...
      { type: 'practice', id: 'd6-p2', lang: 'java', title: 'Practice: Palindrome Checker', starter: 'public class Palindrome {\n    public static void main(String[] args) {\n        String s = "Race car";\n        // TODO: remove spaces, lowercase, check if reversed equals original\n        // Think: which String methods remove unwanted characters?\n        //        which one lowercases? which one reverses?\n    }\n}', hint: 'Clean the string first (strip non-letters, lowercase). Then compare it to its own reverse using .equals().' },
  ...
  // ======== DAY 9: Built-in Methods Toolkit (reference) ========
  {
    id: 'java-14-d9', number: 9,
    title: 'Built-in Methods Toolkit',
    subtitle: 'The String, Arrays & Math reference you will use every single day',
    duration: 60,
    topics: ['String Methods', 'Arrays Utilities', 'Math Helpers', 'Search & Sort Patterns'],
    blocks: [
      { type: 'callout', id: 'd6t-intro', calloutType: 'info', title: 'The Toolkit You Will Reach For Constantly', content: 'This is a pure **reference day** — no practice exercises, just the built-in methods that solve 90% of beginner problems. Bookmark this page. When a practice question says "count characters", "sort the array", or "find the index", these are the tools you use.' },
      { type: 'heading', id: 'd6t-string', level: 2, content: 'String Methods — The Heavy Hitters' },
      { type: 'table', id: 'd6t-string-table', headers: ['Method', 'What It Does', 'Example', 'Result'], rows: [
        ['length()', 'Count characters', '"Hello".length()', '5'],
        ['isEmpty()', 'Is it "" (empty)?', '"".isEmpty()', 'true'],
        ['charAt(i)', 'Get one character at index i', '"Hello".charAt(0)', 'H'],
        ['equals(s)', 'Compare content (ALWAYS use this, not ==)', '"Hi".equals("hi")', 'false'],
        ['equalsIgnoreCase(s)', 'Compare ignoring case', '"Hi".equalsIgnoreCase("hi")', 'true'],
        ['compareTo(s)', 'Compare alphabetically. Negative/0/positive', '"apple".compareTo("banana")', 'negative'],
        ['compareToIgnoreCase(s)', 'Compare alphabetically ignoring case', '"APPLE".compareToIgnoreCase("banana")', 'negative'],
        ['substring(a, b)', 'Grab piece from index a to b (b exclusive)', '"Hello".substring(1, 3)', 'el'],
        ['substring(a)', 'Grab from index a to the end', '"Hello".substring(2)', 'llo'],
        ['contains(s)', 'Does it include this substring?', '"Hello".contains("ell")', 'true'],
        ['startsWith(s) / endsWith(s)', 'Check prefix / suffix', '"Hello".startsWith("He")', 'true'],
        ['indexOf(c or s)', 'Index of first match, or -1 if absent', '"Hello".indexOf(\'l\')', '2'],
        ['lastIndexOf(c or s)', 'Index of LAST match, or -1', '"Hello".lastIndexOf(\'l\')', '3'],
        ['toUpperCase() / toLowerCase()', 'Change case', '"Hi".toUpperCase()', 'HI'],
        ['trim()', 'Remove leading/trailing spaces', '"  hi  ".trim()', 'hi'],
        ['replace(a, b)', 'Replace all occurrences', '"Hi Hi".replace("Hi", "Bye")', 'Bye Bye'],
        ['replaceAll(regex, s)', 'Replace all matching a regex pattern', '"a1b2c3".replaceAll("[0-9]", "")', 'abc'],
        ['split(sep)', 'Break into a String[]', '"a,b".split(",")', '[a, b]'],
        ['repeat(n)', 'Repeat the string n times', '"ab".repeat(3)', 'ababab'],
        ['concat(s)', 'Join two strings (like +)', '"Hi".concat(" there")', 'Hi there'],
        ['toCharArray()', 'Convert to char[] for indexing', '"Hi".toCharArray()', '[H, i]'],
        ['valueOf(x)', 'Convert anything to String', 'String.valueOf(42)', '"42"'],
      ] },
      { type: 'code', id: 'd6t-last-char', lang: 'java', title: 'The "Last Character" Trick', code: 'String s = "Hello";\nchar last = s.charAt(s.length() - 1);  // \'o\'\n// length() - 1 is ALWAYS the last valid index.\n// Trying s.charAt(s.length()) throws StringIndexOutOfBoundsException!' },
      { type: 'callout', id: 'd6t-equals', calloutType: 'warn', title: '== vs equals() — The #1 Trap', content: '`==` compares **references** (are these the same object?). `.equals()` compares **content** (are these the same characters?).\n\n```java\nString a = new String("Hi");\nString b = new String("Hi");\na == b;          // false — two different objects\nb.equals(a);     // true — same content\n```\n\n**Rule: ALWAYS use `.equals()` for strings.** The only exception is string literals from the pool, and even then, just use `.equals()`.' },
      { type: 'heading', id: 'd6t-arrays', level: 2, content: 'Arrays Utilities (import java.util.Arrays;)' },
      { type: 'callout', id: 'd6t-arrays-import', calloutType: 'tip', title: 'Remember the import', content: 'All the array helpers live in `java.util.Arrays`. Add this at the top of your file:\n\n```java\nimport java.util.Arrays;\n```' },
      { type: 'table', id: 'd6t-arrays-table', headers: ['Method', 'What It Does', 'Example', 'Result'], rows: [
        ['Arrays.toString(arr)', 'Print array nicely (avoids [I@1540c19d)', 'Arrays.toString({1,2,3})', '[1, 2, 3]'],
        ['Arrays.sort(arr)', 'Sort ascending (numbers or strings)', 'Arrays.sort({3,1,2})', '[1, 2, 3]'],
        ['Arrays.fill(arr, v)', 'Fill every slot with v', 'Arrays.fill(new int[3], 7)', '[7, 7, 7]'],
        ['Arrays.copyOf(arr, n)', 'Copy, or resize (extra slots = 0/null)', 'Arrays.copyOf({1,2}, 5)', '[1, 2, 0, 0, 0]'],
        ['Arrays.copyOfRange(arr, a, b)', 'Copy a slice (b exclusive)', 'Arrays.copyOfRange({1,2,3,4}, 1, 3)', '[2, 3]'],
        ['Arrays.equals(a, b)', 'Compare two arrays by content', 'Arrays.equals({1,2}, {1,2})', 'true'],
        ['Arrays.binarySearch(arr, v)', 'Find index of v (arr MUST be sorted first!)', 'Arrays.binarySearch({1,2,3}, 2)', '1'],
        ['Arrays.asList(a, b, c)', 'Wrap values as a fixed-size list', 'Arrays.asList("a", "b")', '[a, b]'],
        ['Arrays.stream(arr)', 'Turn array into a stream (Day 29 territory)', 'Arrays.stream({1,2,3}).sum()', '6'],
      ] },
      { type: 'code', id: 'd6t-sort-code', lang: 'java', title: 'Sorting Numbers and Strings', code: 'import java.util.Arrays;\n\nint[] nums = {3, 1, 4, 1, 5};\nArrays.sort(nums);\nSystem.out.println(Arrays.toString(nums));  // [1, 1, 3, 4, 5]\n\nString[] names = {"Ravi", "Aman", "Priya"};\nArrays.sort(names);\nSystem.out.println(Arrays.toString(names));  // [Aman, Priya, Ravi]\n\n// binarySearch REQUIRES a sorted array:\nint index = Arrays.binarySearch(nums, 4);   // 3' },
      { type: 'callout', id: 'd6t-sort-copy', calloutType: 'warn', title: 'sort() Modifies Your Array — copy first if you need the original', content: '`Arrays.sort(arr)` sorts **in place** — it changes the array you pass in. If you need the original order later, copy first:\n\n```java\nint[] original = {3, 1, 2};\nint[] sortedCopy = Arrays.copyOf(original, original.length);\nArrays.sort(sortedCopy);\n// original is still [3, 1, 2]; sortedCopy is [1, 2, 3]\n```' },
      { type: 'heading', id: 'd6t-math', level: 2, content: 'Math Helpers (no import needed)' },
      { type: 'table', id: 'd6t-math-table', headers: ['Helper', 'What It Does', 'Example'], rows: [
        ['Math.max(a, b)', 'Larger of two values', 'Math.max(3, 7) → 7'],
        ['Math.min(a, b)', 'Smaller of two values', 'Math.min(3, 7) → 3'],
        ['Math.abs(n)', 'Absolute value (int, long, double)', 'Math.abs(-5) → 5'],
        ['Math.pow(a, b)', 'a raised to b (returns double)', 'Math.pow(2, 10) → 1024.0'],
        ['Math.sqrt(n)', 'Square root', 'Math.sqrt(16) → 4.0'],
        ['Math.cbrt(n)', 'Cube root', 'Math.cbrt(27) → 3.0'],
        ['Math.round(n)', 'Round to nearest whole', 'Math.round(3.6) → 4'],
        ['Math.floor(n)', 'Round DOWN', 'Math.floor(3.9) → 3.0'],
        ['Math.ceil(n)', 'Round UP', 'Math.ceil(3.1) → 4.0'],
        ['Math.random()', 'Random double in [0, 1)', 'Math.random() → 0.73...'],
        ['Math.log(n) / log10(n)', 'Natural / base-10 log', 'Math.log10(1000) → 3.0'],
        ['Math.sin / cos / tan(rad)', 'Trig functions (radians!)', 'Math.sin(Math.PI / 2) → 1.0'],
        ['Math.PI / Math.E', 'Constants', 'Math.PI → 3.14159...'],
      ] },
      { type: 'code', id: 'd6t-random-range', lang: 'java', title: 'Random Number in a Range (the trick)', code: '// Random int from 1 to 10 (inclusive):\nint dice = (int)(Math.random() * 10) + 1;\n// Math.random() gives [0, 1). ×10 gives [0, 10). +1 gives [1, 11) → 1..10\n\n// General pattern: random int from min to max inclusive:\n// (int)(Math.random() * (max - min + 1)) + min\n\n// Example: random number 50 to 100:\nint score = (int)(Math.random() * 51) + 50;' },
      { type: 'heading', id: 'd6t-patterns', level: 2, content: 'The Loop Patterns You Will Keep Reusing' },
      { type: 'code', id: 'd6t-maxmin', lang: 'java', title: 'Find Max / Min', code: 'int[] nums = {3, 7, 2, 9, 4};\nint max = nums[0];   // start with the first element\nfor (int i = 1; i < nums.length; i++) {\n    if (nums[i] > max) max = nums[i];   // for min: flip > to <\n}\nSystem.out.println(max);  // 9' },
      { type: 'code', id: 'd6t-search', lang: 'java', title: 'Find the Index of a Value', code: 'String[] names = {"Aman", "Priya", "Ravi"};\nString target = "Priya";\nint index = -1;   // -1 = "not found"\nfor (int i = 0; i < names.length; i++) {\n    if (names[i].equals(target)) {\n        index = i;\n        break;   // stop at the first match\n    }\n}\nSystem.out.println(index);  // 1' },
      { type: 'code', id: 'd6t-countsum', lang: 'java', title: 'Count and Sum', code: 'int[] nums = {3, 7, 2, 9, 4};\nint sum = 0;\nint countAbove5 = 0;\nfor (int n : nums) {\n    sum += n;\n    if (n > 5) countAbove5++;\n}\nSystem.out.println("Sum: " + sum);              // 25\nSystem.out.println("Above 5: " + countAbove5);   // 2\n// Average: (double) sum / nums.length  — cast to double for decimals!' },
      { type: 'code', id: 'd6t-reverse', lang: 'java', title: 'Reverse an Array In Place', code: 'int[] nums = {1, 2, 3, 4, 5};\nfor (int i = 0; i < nums.length / 2; i++) {\n    int temp = nums[i];\n    nums[i] = nums[nums.length - 1 - i];\n    nums[nums.length - 1 - i] = temp;\n}\nSystem.out.println(Arrays.toString(nums));  // [5, 4, 3, 2, 1]\n// Only loop to the MIDDLE (length/2) — otherwise you swap back!' },
      { type: 'code', id: 'd6t-charclass', lang: 'java', title: 'Character Classification (no import needed)', code: 'char c = \'A\';\nCharacter.isLetter(c);    // true\nCharacter.isDigit(\'7\');   // true\nCharacter.isWhitespace(\' \'); // true\nCharacter.isUpperCase(\'A\'); // true\nCharacter.isLowerCase(\'a\'); // true\nCharacter.toUpperCase(\'a\'); // \'A\'\nCharacter.toLowerCase(\'A\'); // \'a\'\n\n// Vowel check pattern:\nString vowels = "aeiouAEIOU";\nchar ch = \'e\';\nif (vowels.indexOf(ch) != -1) {\n    System.out.println(ch + " is a vowel");\n}' },
      { type: 'table', id: 'd6t-cheatsheet', headers: ['What you want to do', 'What to use'], rows: [
        ['Count characters', 's.length()'],
        ['Check if empty', 's.isEmpty()'],
        ['Compare strings', 's.equals() or s.compareTo()'],
        ['Slice a string', 's.substring(a, b)'],
        ['Find something', 's.indexOf() or s.contains()'],
        ['Find something from the end', 's.lastIndexOf()'],
        ['Change case', 's.toUpperCase() / s.toLowerCase()'],
        ['Break into array', 's.split(",")'],
        ['Repeat a string', 's.repeat(n)'],
        ['Sort array', 'Arrays.sort(arr)'],
        ['Print array', 'Arrays.toString(arr)'],
        ['Copy / resize', 'Arrays.copyOf(arr, n)'],
        ['Copy a slice', 'Arrays.copyOfRange(arr, a, b)'],
        ['Find index in sorted array', 'Arrays.binarySearch(arr, v)'],
        ['Max / min value', 'Loop with if (or Math.max/min)'],
        ['Find index (unsorted)', 'Loop with if + .equals()'],
        ['Random int in range', '(int)(Math.random() * (max - min + 1)) + min'],
        ['Check letter/digit/space', 'Character.isLetter / isDigit / isWhitespace'],
      ] },
      { type: 'callout', id: 'd6t-closing', calloutType: 'tip', title: 'How to Use This Day', content: 'When you hit a practice question that stumps you, come back here and find the tool. **Knowing the method exists is half the battle** — the other half is reading the docs in your IDE (Ctrl+Click a method) to get the exact signature. You do not need to memorize every method; you need to remember "there is a method for that", then look it up.' },
    ],
    tasks: [
      { id: 'java-14-d9-t1', text: 'Open this page alongside Day 8 practices. Try to name the method BEFORE looking it up.', tag: 'review' },
      { id: 'java-14-d9-t2', text: 'Write a tiny "toolkit playground" file: use every String and Arrays method once, print the results.', tag: 'lab' },
    ],
  },
];
