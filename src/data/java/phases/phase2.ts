...
export const phase2days: Day[] = [
  // ======== DAY 5: Methods (CE-1 Checkpoint) ========
  {
    ...
    blocks: [
      { type: 'callout', id: 'd5-intro', calloutType: 'info', title: 'CE-1 Checkpoint — This Day Matters', content: 'Methods are the first building block of **modular programming**. This day marks the first sessional checkpoint. If you understand methods thoroughly, OOP (Days 7-10) builds naturally on this foundation.' },
      { type: 'heading', id: 'd5-what', level: 2, content: 'What Is a Method?' },
      { type: 'paragraph', id: 'd5-def', content: 'A method is a **named block of code that performs a specific task**. Write it once, **call** it many times. This is **code reuse** — the single most important principle in software engineering.' },
      { type: 'code', id: 'd5-anatomy', lang: 'java', title: 'Method Anatomy', code: '//  access  static?  return   name      parameters\n//    ↓       ↓       ↓       ↓           ↓\n   public  static   int    add (int a, int b) {\n       return a + b;\n   }  // signature = add(int, int)' },
      { type: 'table', id: 'd5-void', headers: ['Aspect', 'void', 'Non-void'], rows: [['Returns?', 'No', 'Yes—must use return'], ['return', 'Optional: return; (exits)', 'Mandatory: return value;'], ['In expression?', 'No', 'Yes: int x = add(3,4);'], ['Use', 'Printing, side effects', 'Calculations, queries']] },
      { type: 'code', id: 'd5-methods', lang: 'java', title: 'Complete Method Example', code: 'public class Calc {\n    public static int add(int a, int b) { return a + b; }\n    public static void print(String s) { System.out.println(s); }\n    public static void main(String[] args) {\n        int sum = add(5, 3);      // non-void — captures result\n        print("5 + 3 = " + sum);  // void — just executes\n    }\n}' },
      { type: 'heading', id: 'd5-pass', level: 2, content: 'Parameter Passing — Pass-by-Value' },
      { type: 'paragraph', id: 'd5-pass-p', content: 'Java is **pass-by-value** for ALL types. For primitives: a COPY of the value. For objects: a COPY of the REFERENCE. Changing the parameter inside the method does NOT affect the original variable — BUT you can modify the object\'s contents through the reference copy.' },
      { type: 'code', id: 'd5-pass-code', lang: 'java', title: 'Pass-by-Value Demo', code: 'static void changePrimitive(int x) { x = 100; }  // local copy\nstatic void modifyArray(int[] arr) { arr[0] = 999; }  // same object!\n\npublic static void main(String[] args) {\n    int n = 10;  changePrimitive(n);\n    System.out.println(n);  // STILL 10 (copy was changed)\n    int[] d = {5,10,15};  modifyArray(d);\n    System.out.println(d[0]);  // 999 (array content modified)\n}' },
      { type: 'callout', id: 'd5-stack', calloutType: 'tip', title: 'Method Call Memory', content: 'Each method call creates a **stack frame** with its own local variables and parameters. When the method returns, the frame is destroyed. Local variables do NOT survive the method call.' },
      // Doubt Clinics
      { type: 'callout', id: 'd5-d1', calloutType: 'doubt', title: "Why can't a method return two values?", content: 'A method can only have ONE return type. Workarounds:\n1. **Return an array or List**: `return new int[]{min, max};`\n2. **Return a custom object**: Create a Result class containing both values.\n3. **Modify parameter objects**: Pass a mutable object and modify its fields.\nThis is a common exam question — the answer is "Java methods return exactly one value."' },
      { type: 'callout', id: 'd5-d2', calloutType: 'doubt', title: 'Actual vs Formal parameters — what\'s the difference?', content: '**Formal parameters** = defined in the method signature: `add(int a, int b)` — `a` and `b` are formal.\n**Actual parameters** (arguments) = values passed during the call: `add(5, 3)` — `5` and `3` are actual.\nThe actual values are copied into the formal parameters when the method is called.' },
      { type: 'callout', id: 'd5-d3', calloutType: 'doubt', title: 'Can a void method have a return statement?', content: '**Yes!** `return;` (with no value) immediately exits the void method. This is useful for early returns:\n```java\nvoid process(int x) {\n    if (x < 0) return;  // exit early — nothing to do\n    // process positive x...\n}\n```' },
      // Exam Alert
      { type: 'callout', id: 'd5-exam', calloutType: 'exam', title: 'Exam Alert — CE-1 Focus', content: '1. **"What happens to local variables after method returns?"** → Stack frame destroyed, variables gone.\n2. **Pass-by-value for both primitives and object references** — tested in code-tracing questions.\n3. **void vs non-void** — CE-1 coding may ask you to write methods of each type.\n4. **Method signature** = method name + parameter types (not return type).' },
      // Bridge
      { type: 'callout', id: 'd5-bridge', calloutType: 'bridge', title: 'Connect the Dots', content: 'Every getter on **Day 7 (Classes/Objects)** is a non-void method. Every setter is a void method. Method overloading on **Day 9 (Polymorphism)** extends this day\'s concepts.' },
      // Quick Ref
      { type: 'table', id: 'd5-qref', headers: ['Concept', 'Key Point'], rows: [['Method signature', 'name + parameter types (NOT return type)'], ['void', 'No return value. Can use return; to exit early.'], ['Non-void', 'Must return value matching declared return type.'], ['Pass-by-value', 'Copy of primitive OR copy of reference — original untouched.'], ['Stack frame', 'Created per call. Destroyed on return. Locals gone.']] },
      // Quiz
  ...
  // ======== DAY 6: Arrays & Strings ========
  {
    ...
    blocks: [
      { type: 'callout', id: 'd6-intro', calloutType: 'info', title: 'Storing Multiple Values', content: 'Single variables hold one value. **Arrays** hold multiple values of the same type. **Strings** are immutable objects with a powerful API. Both are essential for data processing in AIML.' },
      { type: 'heading', id: 'd6-arr', level: 2, content: 'Arrays — Complete Guide' },
      { type: 'code', id: 'd6-arr-code', lang: 'java', title: 'Array Basics', code: '// Declaration + allocation\nint[] marks = new int[5];       // all 0\n\n// Init with values\ndouble[] gpas = {8.5, 9.2, 7.8};\n\n// Access (0-indexed!)\nmarks[0] = 85;  marks[4] = 92;\n\n// length is a field, NOT a method\nSystem.out.println(marks.length);  // 5\n\n// For-each loop\nfor (double g : gpas) System.out.println(g);' },
      { type: 'callout', id: 'd6-arr-gotcha', calloutType: 'warn', title: 'Array Pitfalls', content: '1. **ArrayIndexOutOfBoundsException**: accessing invalid index (e.g., arr[5] when size=5, indices=0-4).\n2. **length is a field**: `arr.length`, NOT `arr.length()` (that is for Strings!).\n3. **Arrays are objects**: passed by reference-copy — modifying inside a method modifies the original.' },
      { type: 'heading', id: 'd6-str', level: 2, content: 'Strings — Immutable Objects' },
      { type: 'paragraph', id: 'd6-str-p', content: '`String` is a **class**, not a primitive. Strings are **immutable** — once created, they cannot change. Any "modification" creates a **new** String object. This is the #1 String exam fact.' },
      { type: 'code', id: 'd6-str-imm', lang: 'java', title: 'String Immutability', code: 'String s1 = "Hello";\nString s2 = s1;\ns1 = s1 + " World";  // NEW String object!\nSystem.out.println(s1); // Hello World\nSystem.out.println(s2); // Hello (unchanged!)' },
      { type: 'table', id: 'd6-str-methods', headers: ['Method', 'Does', 'Example→Result'], rows: [['length()', 'Char count', '"Hi".length() → 2'], ['charAt(i)', 'Char at index', '"Java".charAt(1) → a'], ['substring(i)', 'From i to end', '"Hello".substring(2) → "llo"'], ['substring(i,j)', 'From i to j(excl)', '"Hello".substring(1,4)→"ell"'], ['equals(s)', 'Content equality', '"abc".equals("abc")→true'], ['toLowerCase()', 'Lowercase', '"JAVA".toLowerCase()→"java"'], ['trim()', 'Remove spaces', '" hi ".trim()→"hi"'], ['contains(s)', 'Has substring?', '"hi".contains("i")→true'], ['replace(a,b)', 'Replace all', '"java".replace("a","x")→"jxvx"'], ['split(regex)', 'Split to array', '"a,b".split(",")→["a","b"]']] },
      ...
      // Doubt Clinics
      { type: 'callout', id: 'd6-d1', calloutType: 'doubt', title: 'Why is length a field for arrays but length() a method for Strings?', content: 'Arrays are low-level constructs built into the JVM — their length is stored as part of the array object\'s header, accessed as a field. Strings are high-level objects — `length()` is a method that counts characters. Historical design decision, but the distinction matters: `arr.length` (no parens), `str.length()` (with parens).' },
      { type: 'callout', id: 'd6-d2', calloutType: 'doubt', title: "What's the fastest way to reverse a String?", content: 'Use `StringBuilder` (Day 11): `new StringBuilder(str).reverse().toString()`.\n\n**Never** reverse by concatenating characters in a loop — each `+` creates a new String object, making it O(n²). StringBuilder modifies a single buffer, giving O(n) performance.' },
      { type: 'callout', id: 'd6-d3', calloutType: 'doubt', title: 'What happens if I access arr[-1] or arr[arr.length]?', content: '**ArrayIndexOutOfBoundsException at runtime** — not compile time! The compiler does not check index bounds. This is a common source of runtime crashes. Always verify: `if (index >= 0 && index < arr.length)` before accessing.' },
      // Exam Alert
      { type: 'callout', id: 'd6-exam', calloutType: 'exam', title: 'Exam Alert', content: '1. **String immutability** — #1 String theory question.\n2. **== vs .equals()** — #1 trick question. ALWAYS .equals() for content.\n3. **Array defaults**: int→0, boolean→false, object→null.\n4. **arr.length (field) vs str.length() (method)** — classic MCQ trap.' },
      // Bridge
      { type: 'callout', id: 'd6-bridge', calloutType: 'bridge', title: 'Connect the Dots', content: 'Arrays (fixed size) → **ArrayList (dynamic, Day 14)**. StringBuilder (briefly mentioned) → **Day 11 deep dive**. String immutability → important when using Strings as HashMap keys in collections.' },
      // Quick Ref
      { type: 'table', id: 'd6-qref', headers: ['Concept', 'Key Point'], rows: [['Array declaration', 'int[] arr = new int[n]; OR int[] arr = {1,2,3};'], ['Array access', '0-indexed. arr[0]=first, arr[arr.length-1]=last'], ['String immutable', 'Any modification creates NEW String object'], ['== vs equals', '== = reference. equals() = content. USE equals()'], ['String pool', 'Literals go to pool and are reused. new String() bypasses.']] },
      // Quiz
