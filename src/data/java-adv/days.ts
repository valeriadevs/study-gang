...
public class BatchInsert {
  ...
  // ================================================================
  {
    ...
    blocks: [
      { type: 'callout', id: 'ja8-intro', calloutType: 'info', title: 'The Dark Side of Concurrency', content: 'Multiple threads sharing data = **race conditions**. Two threads read the same counter, both increment, both write back — one increment is LOST. Today you will learn to use `synchronized`, `volatile`, `wait()/notify()`, and the modern **ExecutorService** for thread pools. These are CE-3 and viva essentials.' },
      ...
      { type: 'table', id: 'ja8-qref', headers: ['Concept', 'Key Point'], rows: [
        ...
        ['synchronized method', 'Locks `this`. Only one thread executes any synchronized method on same object.'],
        ['synchronized block', 'Locks specified object. Finer-grained than method. `synchronized(lock) { ... }`'],
        ['volatile', 'Visibility guarantee only. Reads always from main memory. NOT atomic for compound ops.'],
        ['wait()', 'Releases lock, waits for notify. INSIDE synchronized. Always in while() loop.'],
